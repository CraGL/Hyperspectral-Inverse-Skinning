...
void usage( const char* argv0 ) {
    std::cerr<<"Usage:"<<std::endl<<"    " << argv0 << " pos1.obj pos2.obj pos3.obj ... weights.DMAT \
...
int main(int argc, char * argv[])
{
	...
	vector<string> mesh_paths(args.begin(), args.end()-1);
	string weight_path = args.at(args.size()-1);
	
	g_scene.init_scene( mesh_paths, weight_path );
	g_scene.show_pose(0);
    ...
    std::string title = os_path_splitext(os_path_split(mesh_paths[0]).second).first;
	...
	string pose_selection_text = "pos1";
	for(int i=2; i<=mesh_paths.size(); i++)		pose_selection_text += (",pos"+to_string(i));
