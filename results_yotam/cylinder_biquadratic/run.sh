...
INITIAL_GUESS_ARGS="--svd_threshold 1e-15 --transformation_percentile 10 --version 0"
FLAT_INTERSECTION_ARGS="--forced-init True --energy biquadratic --W-projection normalize --strategy pinv+ssv:weighted --x-eps 0 --max-iter 0 --f-eps 0 -GT "${OTHER_POSE_DIR}" --error True --handles 4 --visualize True --visualize-port 9988 --fancy-init "${OUTPUT_DIR}"/local_subspace_recover.txt"
