...
# SIMPLEX_HULL_ARGS="-D 10 --method qp-major --test True"
SIMPLEX_HULL_ARGS="-D 10 --method qp-major --random-percent 10 --random-reps 10 --robust-percentile 1"
...
python -u "${ROOT_DIR}"/simplex_hull_with_initial_guess.py "${OUTPUT_DIR}"/local_subspace_recover.txt "${REST_POSE}" "${OTHER_POSE_DIR}" "${OUTPUT_DIR}"/result.txt ${SIMPLEX_HULL_ARGS} 2>&1 | tee -i "${OUTPUT_DIR}"/simplex_hull.out
