...
for name in "${gt_models[@]}"; do
	...
	do
		...
		# for initial_method in "euclidian"; do 	
			...
			python3 -u PerVertex/local_subspace_recover.py ${INITIAL_GUESS_ARGS} "${MODEL_DIR}"/"${REST_POSE}" "${POSES_DIR}"/*.obj -rand $initial_method -o "${OUTPUT_DIR}"/local_subspace_recover_${initial_method}.txt --save-dmat "${OUTPUT_DIR}" 2>&1 | tee "${OUTPUT_DIR}"/local_subspace_recover.out
