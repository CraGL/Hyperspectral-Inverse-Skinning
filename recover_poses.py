...
def match_data( gt_data, target ):
	...
	def match_criteria(A, B):
		'''
		Measure how different is B from A
		'''
		assert( A.shape == B.shape )
		dist = np.array([np.linalg.norm(pt_gt - pt_data) for pt_gt, pt_data in zip(A, B)]).mean()
# 		dist = np.linalg.norm( A - B )
		return dist
	
	ordering = []
	N = gt_data.shape[0]
	match_board = np.zeros((N,N))
	for i in range(N):
		for j in range(N):
			match_board[i,j] = match_criteria( gt_data[i], target[j] )
