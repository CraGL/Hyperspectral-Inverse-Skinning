...
for REST_POSE in $(basename "${MODEL_DIR}"/wolf*.obj)
do
	...
	do
		...
		H=${test_dir#$name-}
		FLAT_INTERSECTION_ARGS_NO_INIT="--error True -GT ${MODEL_DIR}/${name} --energy biquadratic --CSV ${OUTPUT_DIR}/${test_dir}_no_init.csv --max-iter ${MAXITER} --f-eps 0 --handles ${H} --strategy pinv+ssv:weighted"
