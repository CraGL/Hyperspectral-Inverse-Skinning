...
REST_POSE="${ROOT_DIR}"/models/elephant-gallop.obj
OTHER_POSE_DIR="${ROOT_DIR}"/models/elephant-gallop
# SSD_RESULT="${ROOT_DIR}"/SSD_res/elephant-gallop-27-output.txt
SSD_RESULT="${ROOT_DIR}"/SSD_res/elephant-gallop-27-unconstrained.txt

INITIAL_GUESS_ARGS="--svd_threshold 1e-15 --transformation_threshold 1e-4 --version 0"
FLAT_INTERSECTION_ARGS="--energy biquadratic --W-projection normalize --max-iter 2 --handles 27 --fancy-init ${OUTPUT_DIR}/local_subspace_recover.txt"
# SIMPLEX_HULL_ARGS="--method qp-major"
SIMPLEX_HULL_ARGS="--method qp-major --random-percent 10"
...
# Evaluate
python -u "${ROOT_DIR}"/recover_poses.py --SSD "${SSD_RESULT}" "${REST_POSE}" "${OTHER_POSE_DIR}" "${OUTPUT_DIR}"/result.txt 2>&1 | tee "${OUTPUT_DIR}"/compare.out
