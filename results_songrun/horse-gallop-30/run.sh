...
REST_POSE="${ROOT_DIR}"/models/horse-gallop.obj
OTHER_POSE_DIR="${ROOT_DIR}"/models/horse-gallop
# SSD_RESULT="${ROOT_DIR}"/SSD_res/horse-gallop-30-output.txt
SSD_RESULT="${ROOT_DIR}"/SSD_res/horse-gallop-30-unconstrained.txt

INITIAL_GUESS_ARGS="--svd_threshold 1e-15 --transformation_threshold 1e-4 --version 0"
FLAT_INTERSECTION_ARGS="--energy biquadratic --W-projection normalize --max-iter 2 --handles 30 --fancy-init ${OUTPUT_DIR}/local_subspace_recover.txt"
