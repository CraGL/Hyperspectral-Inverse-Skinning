...
REST_POSE="${ROOT_DIR}"/models/pdance.obj
OTHER_POSE_DIR="${ROOT_DIR}"/models/pdance
# SSD_RESULT="${ROOT_DIR}"/SSD_res/lion-poses-10-output.txt
SSD_RESULT_UNCONSTRAINED="${ROOT_DIR}"/SSD_unconstrained/pdance-24-unconstrained.txt

INITIAL_GUESS_ARGS="--svd_threshold 1e-15 --transformation_threshold 1e-4 --version 0"
FLAT_INTERSECTION_ARGS="--strategy ssv:weighted --energy biquadratic --W-projection normalize --x-eps 0.1 --handles 24 --fancy-init ${OUTPUT_DIR}/local_subspace_recover.txt"
