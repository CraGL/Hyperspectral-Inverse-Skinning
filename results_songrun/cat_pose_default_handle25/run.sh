...
SSD_RESULT="${ROOT_DIR}"/SSD_res/cat-25-output.txt
SSD_RESULT_UNCONSTRAINED="${ROOT_DIR}"/SSD_res/cat-poses-25-unconstrained.txt


INITIAL_GUESS_ARGS="--svd_threshold 1e-15 --transformation_threshold 1e-4 --version 0"
FLAT_INTERSECTION_ARGS="--energy biquadratic --W-projection normalize --max-iter 2 --handles 25 --fancy-init ${OUTPUT_DIR}/local_subspace_recover.txt"
