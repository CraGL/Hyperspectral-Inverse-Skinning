...
do
 ...
 # #    #### if print-all is True, then all qs_data are used for flat_intersection.py
	# # python -u PerVertex/local_subspace_recover.py ${INITIAL_GUESS_ARGS} "${REST_POSE}" "${POSES_DIR}"/*.obj --print-all True --out "${OUTPUT_DIR}"/local_subspace_recover_100percent.txt --out-errors "${OUTPUT_DIR}"/local_subspace_recover_errors_100percent.txt --out-ssv "${OUTPUT_DIR}"/local_subspace_recover_ssv_100percent.txt  2>&1 | tee "${OUTPUT_DIR}"/local_subspace_recover_100percent.out
	# # # ## laplacian
	# # FLAT_INTERSECTION_ARGS_LAPLACIAN="--energy laplacian --forced-init True --CSV ${OUTPUT_DIR}/${test_dir}_laplacian.csv --max-iter ${MAXITER} --f-eps 0 --handles ${H} --fancy-init ${OUTPUT_DIR}/local_subspace_recover_100percent.txt --fancy-init-errors ${OUTPUT_DIR}/local_subspace_recover_errors_100percent.txt --fancy-init-ssv ${OUTPUT_DIR}/local_subspace_recover_ssv_100percent.txt"
	# # python3 -u flat_intersection.py "${REST_POSE}" "${POSES_DIR}" ${FLAT_INTERSECTION_ARGS_LAPLACIAN} 2>&1 | tee -i "${OUTPUT_DIR}"/flat_intersection_laplacian_100percent.out
